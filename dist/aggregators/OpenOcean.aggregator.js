...
export default class OpenOceanAggregator extends Base {
            ...
            const quotes = data?.routes
                ...
                const meta = {
                    ...
                    platformFee: route?.fees?.bridgeFee?.amount
                        ? `${Number(ethers.utils.formatUnits(route?.fees?.bridgeFee?.amount, route?.fees?.bridgeFee?.decimals))?.toFixed(6)} ${route?.fees?.bridgeFee?.symbol}`
                        : 0,
    ...
    async getTransactionData(data, restProps, meta) {
