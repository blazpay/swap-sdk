...
export default class Base {
    ...
    async setAllowance(tokenAddress, approvalAddress, provider, chainId, amount = ethers.constants.MaxUint256, router) {
        ...
        if (Number(chainId) === 728126428) {
            ...
            try {
                const encodedTokenAddress = this.tronWeb.address.fromHex("41" + tokenAddress.substring(2));
                const encodedApprovalAddress = router === "nitro"
                    ? this.tronWeb.address.fromHex("41" + approvalAddress.substring(2))
                    : approvalAddress;
                const contract = await this.tronWeb.contract(tokenAbi, encodedTokenAddress);
        ...
        const allowance = await erc20.allowance(this.senderAddress, approvalAddress);
        if (allowance.lt(amount)) {
    ...
    async triggerContract(chainId, provider, txn) {
        try {
            if (Number(chainId) === 728126428) {
